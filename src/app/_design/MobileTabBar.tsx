 ...
 *    그냥 하단에서 탁탁탁 눌러야지」
 *
 * 다섯 기능은 상품 · 접수 · 계약 · 실적 · 정산이다. 실적은 접수 원장의
 * 분납실적/완납실적을 같은 판에서 거른 보기이며, 정산 안에서 청구/지급을 가른다.
...
const 아이콘: Record<string, React.ReactNode> = {
  ...
  실적: <><path d="M4 19V10M10 19V5M16 19v-7M22 19H2" /></>,
...
const TABS = [
  ...
  ['접수', '/intake?v=work'],
  ['계약', '/esign'],
  ['실적', '/intake?scope=performance&iv=all&v=work'],
  ['정산', '/settlement'],
] as const;
...
function useCurrent(): string {
  ...
  if (path.startsWith('/intake')) return sp.get('scope') === 'performance' ? '실적' : '접수';
