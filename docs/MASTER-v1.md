...
## 1. ADMIN 화면 골격 — 1:1:1
PC의 핵심 업무 화면은 **LIST / DETAIL / WORK 판 문법을 공유하되 메뉴 목적에 따라 배열한다.** 정확한 비율·배열·모바일 depth는 `docs/ui/ADMIN-UI-UX-SSOT.md`가 정본이다. 과거의 「같은 폭 세 패널」은 일반 규칙으로 사용하지 않는다.
...
접수목록은 세로 스크롤 중심이며 각 카드/행에서 고객, 차량, 접수번호, 핵심 조건, 계약서/서류/인도 체크를 빠르게 파악한다. 접수 항목을 **한 번 선택하면** 같은 업무 문맥의 상세/업무 판이 접수상세로 전환된다.
