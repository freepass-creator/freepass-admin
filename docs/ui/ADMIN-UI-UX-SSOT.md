...
내부 Admin의 일반 목록 카드는 **Main / Key / Support 3줄**을 사용한다.
줄 수를 늘리지 않고, 세 줄 안에서 필수정보만 표현한다.
...
- 각 줄은 한 줄 유지
- 긴 값은 ellipsis
- 상태 Badge는 Main 안에 포함하고 별도 줄을 만들지 않는다

역할:
- Main: 대상을 가장 빨리 식별하는 정보 + 필요 시 대표값
- Key: 선택 전에 꼭 필요한 핵심조건
- Support: 보조조건 또는 반드시 보여야 하는 정확값

줄바꿈:
- identity/title: nowrap + ellipsis
- 핵심조건: nowrap + ellipsis
- 금액/status: nowrap
- 긴 설명은 상세에서만 자연 줄바꿈

금지:
- 일반 목록에 4번째 정보줄 추가
- 목록 카드 높이를 정보량에 따라 계속 늘리기
- 상세정보를 목록에 과도하게 복제
- 역할별 line-height/gap을 화면마다 다르게 만들기

Visual QA:
- Main / Key / Support 순서와 개수 검사
- 각 정보줄 line-height 20px 검사
- 한 정보줄이 세로 wrap되면 FAIL
...
- 남는 금액/수익: 목록 기본정보에서 제외. 상세/정산 분석에서 필요 시 정확 원 단위
...
내부 Admin의 일반 목록 카드는 **3줄**을 기본이 아니라 정본으로 사용한다.
목록은 대상을 알아보고 빠르게 클릭하는 화면이며, 완전한 정보는 상세가 책임진다.

공통 슬롯:
1. Main — 대상 identity + 상태 + 필요 시 우측 대표값
2. Key — 선택 전에 꼭 필요한 핵심조건
3. Support — 보조조건 또는 반드시 보여야 하는 정확값
...
## 접수
- Main: 고객명/진행상태 ↔ `월 N만 원`
- Key: 차량번호 · 차종 · 상품 · 기간
- Support: `수수료 청구 N원 · 지급 N원`

## 실적
- Main: 고객명/분납·완납 상태
- Key: 차량번호 · 차종 · 상품 · 기간
- Support: `수수료 청구 N원 · 지급 N원`
- 수익/남는 금액은 목록에 두지 않는다.

## 정산 거래처
- Main: 공급사/영업채널 + 상태 ↔ 정산 정확 금액
- Key: 청구서/지급명세 진행
- Support: 정상일 때 완료 건수, 이슈가 있으면 미확정/끊김/환수 요약

## 정산 개별 건

청구 탭:
- Main: 고객명/청구단계 ↔ 청구 수수료 정확 금액
- Key: 차량번호 · 차종 · 상품 · 기간
- Support: 지급 수수료 정확 금액

지급 탭:
- Main: 고객명/지급단계 ↔ 지급 수수료 정확 금액
- Key: 차량번호 · 차종 · 상품 · 기간
- Support: 청구 수수료 정확 금액

## 전자계약
- Main: 고객명/서명상태 ↔ `월 N만 원`
- Key: 차량번호 · 차종
- Support: 기간 · 계약상태

## 절대 규칙
- 일반 목록 카드에 4번째 정보줄을 만들지 않는다.
- 상태는 별도 줄이 아니라 Main의 compact Badge.
- 대여료/보증금은 목록에서 약식 허용.
- 수수료/청구/지급/정산 금액은 표시 시 정확 원 단위.
- 더 보고 싶은 정보는 상세에서 확인한다.

# 68. Three-line list grammar

일반 목록 카드의 기본 문법은 **3줄**이다.

1. **Main**
   - 대상을 가장 빨리 알아보는 identity
   - 우측에는 필요 시 primary value
   - 상태는 별도 줄을 만들지 않고 compact Badge로 Main 옆에 둔다
   - 14px / 600 / strong

2. **Key**
   - 선택 전에 꼭 알아야 하는 핵심 조건
   - 차량번호 · 차종 · 상품 · 기간 등
   - 14px / 500 / secondary

3. **Support**
   - 보조 조건 또는 정확 금액
   - 수수료처럼 목록에서도 반드시 보여야 하는 값이 여기에 올 수 있다
   - 12px / 400 / muted
   - 단, 숫자/금액 자체는 tabular + 우측 정렬 가능

예외:
- 일반 내부 Admin 목록 카드에는 줄 수 예외를 만들지 않는다.
- 이슈도 Support 슬롯 안에서 표현한다.
- 더 많은 정보는 상세로 보낸다

핵심:
> Main / Key / Support는 줄 수보다 의미를 고정하는 규격이다.
> 상태는 중요한 정보지만 별도 한 줄을 차지하는 것이 아니라 Main을 보조한다.

# 69. List visual tile

모든 일반 목록 카드는 왼쪽 **64×64 visual tile**을 공유한다.
...
- 같은 64×64 자리
- 중앙 상태 아이콘 24px
- 아이콘 아래 상태 텍스트 12px / 600
- 접수/실적/정산/전자계약 등에서 동일 geometry

중복 상태 신호:
- 상태타일이 있으면 Main 옆 상태 Badge는 기본적으로 표시하지 않는다.
- 사진 썸네일처럼 상태를 말하지 않는 visual에서는 필요 시 compact Badge 허용.

정렬:
- 64px visual tile 높이 = 3줄 본문 높이
- Main 20 + gap 2 + Key 20 + gap 2 + Support 20 = 64px
- visual tile과 텍스트 블록은 수직 중앙 정렬

목표:
> 왼쪽 위치만 보면 사진/상태를 인지하고, 오른쪽 3줄만 보면 대상을 식별해 바로 클릭할 수 있어야 한다.

# 70. Cross-shell list content parity

Web RowCard와 Mobile ListRow는 같은 3줄 의미를 공유한다.

공통 구조:
- Main 좌측: 대상 identity
- Main 우측: 대표값이 있을 때 primary value
- Key: 분류/식별에 필요한 핵심조건
- Support: 보조값 또는 반드시 보여야 하는 정확값
- 왼쪽 visual: 64×64 사진 또는 상태타일

업무별 대표값:
- 상품: 월 대여료 약식
- 접수: 월 대여료 약식
- 실적: 기본 representative money 없음
- 청구 개별 건: 청구 수수료 정확 금액
- 지급 개별 건: 지급 수수료 정확 금액
- 정산 거래처: 정산 정확 금액
- 전자계약: 월 대여료 약식

정산 반대축:
- 청구 카드 Support = 지급 수수료 정확 금액
- 지급 카드 Support = 청구 수수료 정확 금액
...
- 좌측: 기간
- 중간: 월 대여료 정확금액
- 우측: 보증금 · 선납금 · 연간주행거리 등 선택에 직접 필요한 조건
- 선택된 카드는 tint + inset, 외곽선 없음
- 체크 아이콘/「선택됨」 문구는 기본적으로 사용하지 않는다
- 카드 자체의 selected surface가 선택 상태를 전달한다
- 선택 후 하단 `접수하기`는 선택된 offer id를 사용
- Web/Mobile 모두 같은 한 줄 의미 구조를 사용한다
- 모바일에서도 기간 버튼 + 별도 선택값 요약으로 이중 표현하지 않는다
