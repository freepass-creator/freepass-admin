...
- 드롭다운: 정산관리 «정산월» · 전자계약 «계약상태» 하나씩만.
- 대표 2026-09-23 「규격 잘 잡아놓고 조금만 다른 거 얘기했더니 없는 거 얘기했던 거 깨지냐 · 위에 대수 몇 대 그런 거 필요 없어」
  → 말하지 않은 것은 규격 그대로 둔다: ai-core 시안 · 템플릿을 원래대로 되돌리고(검색창 · 상세 필터는 추가 부품으로만),
  관리자도 설명 문장 · 머리 단추 · 상태 탭 줄을 규격대로 되돌렸다. 바뀐 것은 ⑥ 조회(검색창 + 상세 필터)와 드롭다운 수,
  그리고 상품찾기의 대수 KPI 삭제뿐이다.
- 대표 2026-09-23 「요약 카드는 필요 없다고 … 접수 밑에 검색창 나오고 목록 나오면 되잖아」
  → 모든 PC 화면에서 요약 카드(KPI)와 정산 흐름 건수 카드를 뺐다. 제목 → 검색창 → 목록.
- 대표 2026-09-23 「목록을 카드 타입처럼 … 핸드폰 모바일 버전으로 넘어가기 쉽게」
  → 네 화면(상품 · 접수/실적 · 정산 · 전자계약)의 목록을 표에서 카드형(ai-core 규격 §5-2 `erp-cardlist` · `erp-listcard`)으로 바꿨다.
- 대표 2026-09-23 「줄 타입은 촌스럽다 … 카드를 기다랗게」 · 「그거 규격으로 다 해놓고」
  → 목록 기본을 긴 카드(ai-core 규격 §5-3 `erp-rowcard`)로 바꿨다. ① 누구 · 무슨 차 → ② 작은 진행 단계 → ③ 조건 → ④ 돈. 상품은 단계 없이(`--no-steps`).
    카드 칸 수는 넓이에 맞춰 줄어든다(좁으면 한 칸) — 같은 마크업이 폰까지 간다. 합계는 목록 아래 바닥 줄. 폰은 아직 옛 판(전환은 다음 단계).
