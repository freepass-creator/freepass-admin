    ...
    <div className="fn-box">
      ...
      <form id={progressFormId(code, 'delivered')} onSubmit={send} className="dz-progress-row" aria-busy={pending}>
        ...
        {delivered && <button name="on" value="0" className="dz-action-danger"
          ...
          disabled={pending || cancelled || disabled}>인도 되돌림</button>}
      </form>
      <form id={progressFormId(code, 'cancelled')} onSubmit={send} aria-busy={pending}>
        ...
        {cancelled
          ...
          : <>취소 사유 <input name="reason" size={30} disabled={pending || disabled} /> <button name="on" value="1" className="dz-action-danger"
