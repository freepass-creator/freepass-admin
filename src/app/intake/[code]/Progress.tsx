  ...
  return (
    <div className="fn-box">
      <form id={progressFormId(code, 'plate')} onSubmit={send} className="dz-progress-row" aria-busy={pending}>
        ...
        차량번호 <input name="plate" defaultValue={plate} placeholder="배정 후 입력" disabled={pending || cancelled || disabled} />{' '}
        <button type="submit" disabled={pending || cancelled || disabled}>차량번호 저장</button>
      </form>
      <form id={progressFormId(code, 'paper')} onSubmit={send} className="dz-progress-row" aria-busy={pending}>
        ...
        계약서 {paper ? '● 받음' : '○ 안 받음'}{' '}
        <button name="on" value={paper ? '0' : '1'} disabled={pending || cancelled || disabled}>{paper ? '받음 해제' : '받음으로'}</button>
      </form>
      <form id={progressFormId(code, 'delivered')} onSubmit={send} className="dz-progress-row" aria-busy={pending}>
        ...
        <input type="date" name="deliveredAt" defaultValue={deliveredAt || today} disabled={pending || cancelled || disabled} />{' '}
        <button name="on" value="1" disabled={pending || cancelled || disabled}>{delivered ? '인도일 고침' : '인도 완료'}</button>{' '}
        {delivered && <button name="on" value="0" disabled={pending || cancelled || disabled}>인도 되돌림</button>}
      </form>
      <form id={progressFormId(code, 'cancelled')} onSubmit={send} aria-busy={pending}>
        ...
        {cancelled
          ? <>● 취소됨 <button name="on" value="0" disabled={pending || disabled}>취소 풀기</button></>
          : <>취소 사유 <input name="reason" size={30} disabled={pending || disabled} /> <button name="on" value="1" disabled={pending || disabled}>취소</button></>}
      </form>
