...
if (APPLY) {
  ...
  await db.collection('settlement_rules').doc('current').set({
    ...
    notes: {
      ...
      clawback: '환수는 접수 줄의 체크가 아니라 settlement_clawbacks 의 «반대 부호 한 줄» — 환수일이 든 달·그 상대에서 뺀다 · 금액·사유는 사람이 넣는다(조건이 공급사마다 다르다) · id 차번_환수월 · 인도된 줄만 (대표 2026-09-18 「환수 열어두고」)',
      manual: 'auto=false 규칙(건별 책정·최대 9%·구독료+정액·조건분기)·표에 없는 공급사는 기계가 금액을 내지 않는다 — 접수 때 사람이 직접 넣는다(넣은 값이 이긴다) · 표와 다르게 넣으면 사유 필수 · 접수 뒤 고치기도 사유 필수(청구서·지급명세 나간 쪽은 못 고침)',
      claimLink: '청구 링크(PDF 대신) — 발행한 청구서에 링크 · 토큰(해시만 저장) + 상대 사업자등록번호가 맞아야 열림 · 10번 틀리면 30분 잠금 · 거두면 영영 안 열림 · 상대는 굳힌 사본(자기 쪽 금액만 · 손님 이름 가림)을 보고 확인/이의 · 이의면 그 줄 정정 (대표 2026-09-18)',
      intake: '접수 두 길 — 차 골라서 접수(상품·요금에서 채움) · 직접 접수(손으로) (대표 2026-09-18) · 흐름 접수 → 완납실적 → 청구 → 지급 · 환수는 열어 둔다',
    },
  }, { merge: false });
