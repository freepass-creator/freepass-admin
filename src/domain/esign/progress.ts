...
export type EsignState = '미발송'|'발행'|'열람'|'진행중'|'검토대기'|'서명완료'|'반려'|'만료'|'해지';
...
export function esignStage(session: EsignSession|null|undefined, now=Date.now()):EsignStage {
  ...
  if(session.status==='revoked') return {...base,state:'해지',done:0,label:'해지',tone:'red'};
  ...
  if(session.status==='pending_review'||session.status==='approving') return {...base,state:'검토대기',done:total-1,label:'검토대기',tone:'amber'};
...
export function adminStage(session: EsignSession|null): '작성'|'발송 전'|'고객 작성 중'|'검토 대기'|'완료' {
  ...
  if(session.status==='pending_review'||session.status==='approving') return '검토 대기';
