...
### 13-6. 관리자 5기능 진입점 — 2026-09-22 사용자 확정

- 웹과 모바일의 전역 기능 순서는 `상품찾기 / 계약접수 / 전자계약 / 실적관리 / 정산관리`로 통일한다.
- 모바일 라벨은 `상품 / 접수 / 계약 / 실적 / 정산`을 쓴다.
- 실적관리는 새 원장을 만들지 않고 접수 원장의 `분납실적 / 완납실적`만 거른다.
- 청구와 지급은 별도 전역 메뉴로 분리하지 않고 정산관리 안의 업무 모드로 유지한다.
- `/`의 첫 진입은 상품찾기(`/products`)다.
