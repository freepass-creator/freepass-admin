/**
 * ★아이콘 한 벌 — erp4 가 쓰는 lucide 글리프를 «그 모양 그대로» 옮겼다(ISC 라이선스 · 24 격자 · 획 2 · 끝 둥글게).
 *   이 집에는 lucide 가 안 깔려 있어 경로만 옮겨 적는다. 새로 그리지 않는다.
 *   ⓘ erp4 규칙(대표 2026-08-28 「박스 뱃지 쓰지 말고 아이콘 텍스트 형태로 · 모든 곳에서」 ·
 *     08-20 「아이콘은 섹션 머리에 하나씩만 — 칸 안에는 넣지 않는다」)을 같이 가져온다.
 *   색은 부르는 쪽 글자색(currentColor)을 따른다.
 */
...
export function Icon({ name, size = 14, stroke = 2 }: { name: string; size?: number; stroke?: number }) {
  ...
  if (!g) return null;
  return (
    <svg className="dz-ico" width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor"
      strokeWidth={stroke} strokeLinecap="round" strokeLinejoin="round" aria-hidden>{g}</svg>
  );
}
