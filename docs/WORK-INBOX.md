...
| ④ | **고른 것·강조는 색으로만** — 켜진 탭·필터·완료 = 검은 면, 고른 카드 = 짙은 면, 가장 센 강조(선택 Offer) = 검은 박스. 테두리·고리·그림자로 강조하지 않는다 | 「선택된 거나 강조해야 할 건 색깔로만 위계」 |
| ⑤ | **판·목록·표는 박스** — 회색 바탕 위 흰 판(선 있음), 판 위 옅은 회색 카드(선 없음). 표도 칸마다 박스 | 「다 카드형 카드버튼형으로 통일하고 표도」 · 「패널 나눠 놓은 것도 카드형으로」 · 「박스에 담아 넣자」 |

구현 = `src/app/globals.css` 맨 끝 「대표 확정 2026-09-18」 덩이. 토큰 `--r:4px` · `--바닥` · `--판` · `--박스` · `--박스손` · `--고름` · `--켜짐` · `--선`.
글꼴 = Inter/Pretendard. 판 셋 = 상품 목록 | 상품 상세 | 접수 목록.
