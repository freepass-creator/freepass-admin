...
| ⑧ | **폰은 판을 한 장씩** — 아래 탭(상품·상세·접수). 차를 누르면 상세, 접수하기는 접수로 넘어간다. 폰 입력칸만 16px(iOS 확대 막기) | 「모바일 웹 적용해봐」 |
...
### 14-0-2. 뱃지는 두 가지 — 화이트라벨 규격 (대표 2026-09-18 「뱃지 규격이 왜 다르지? 화이트라벨 거 갖고 와」)

정본 = freepasserp4 `components/shop/shop-ui.tsx`. 부품 = `src/app/_design/Badges.tsx`.
- ① **신원 딱지**(상품구분 · 배차상태 · 할 일) = 상자 하나 규격 — 22px · 라운드 4 · 12px. 뜻은 색으로만(회색 / 할 일=옅은 남색).
- ② **조건 표시**(무심사 · 분납가능 · 만21세 …) = **상자 없이 ✓ + 굵은 먹색 글자**(색은 ✓ 에만). 신원 딱지와 한눈에 갈린다.
