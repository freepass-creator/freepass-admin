...
목록 카드의 목적은 **대상을 알아보고 바로 선택하는 것**이다.
완전한 업무 정보는 상세 화면이 책임진다.

전역 최대 줄 수는 두지 않지만, 일반 목록은 **3~4줄 중심**으로 유지한다.
줄을 더 추가하려면 그 정보가 선택 전에 반드시 필요한 이유가 있어야 한다.
...
**수익/남는 금액은 목록 기본정보에 넣지 않는다.** 상세에서 확인한다.

## 정산 거래처 카드 — 표준 2줄 + 이슈 1줄 조건부

1. 공급사/영업채널 + 상태 ↔ 우측 **정산 N원 정확 금액**
2. 문서 진행 · 완료 건수
3. 미확정/끊김/환수가 있을 때만 이슈 줄 추가

## 정산 개별 건 — 3줄, 현재 축 중심

### 청구 탭
1. Main: 고객명 + 현재 단계 ↔ 우측 **청구 수수료 N원**
2. Key: 차량번호 · 차종 · 상품 · 기간
3. Support: **지급 수수료 N원**

### 지급 탭
1. Main: 고객명 + 현재 단계 ↔ 우측 **지급 수수료 N원**
2. Key: 차량번호 · 차종 · 상품 · 기간
3. Support: **청구 수수료 N원**

청구 화면에서는 청구 수수료가 주값이고 지급 수수료는 비교용 보조값이다.
지급 화면에서는 그 반대다. 둘을 같은 강도로 병렬 표시하지 않는다.
...
계약코드/담당/보험/날짜는 상세에서 확인한다.

## 공통 원칙

- 목록 = 식별 + 선택 + 빠른 비교.
- 상세 = 완전한 업무정보.
- 첫째 줄은 그 대상을 가장 빨리 알아보는 정보.
- 대여료·보증금만 목록에서 만원 단위 약식 허용.
- 수수료/청구/지급/정산 금액은 표시할 때 정확 원 단위.
- 정산 개별 카드는 현재 축의 수수료를 Main value로, 반대 축 수수료를 Support로 둔다.
- 수익/마진은 목록 기본 필드가 아니다.
- 숫자/금액은 우측 정렬 + tabular-nums.
- 단순히 데이터가 있다는 이유로 목록 줄을 추가하지 않는다.
- 이슈는 선택 전에 알아야 할 때만 조건부 한 줄 추가.
...
예외:
- 거래처 정산처럼 정보가 충분하면 2줄도 허용
- 경고/이슈는 선택 전에 반드시 알아야 할 때만 조건부 추가 1줄
- 상세정보 때문에 일반 카드를 4~5줄로 늘리지 않는다
