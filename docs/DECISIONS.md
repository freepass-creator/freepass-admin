...
## DEC-2026-09-26-03 — 운영입력 컨트롤은 값의 성격에 맞춘다
상태: USER CONFIRMED / ADOPTED

F04 운영 시트의 실제 입력 방식을 Admin UX에 반영한다.

### 체크형 사실
YES/NO 관측 사실은 체크박스로 보이고 조작한다.
- 계약서
- 인도완료

단, 접수취소/계약취소/환수처럼 사유·금액·날짜가 필요한 위험 업무는 단순 체크박스로 축소하지 않는다.

### 드롭다운 분류
분류값은 자유입력보다 제한된 선택으로 받는다.
- 상품구분
- 분납여부
- 렌트구분
- 계약형태
...
### 시스템 계산 상태
청구상태·실적상태·정산상태는 사람이 임의 드롭다운으로 덮어쓰지 않는다.
