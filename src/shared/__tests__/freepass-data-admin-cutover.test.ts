...
test('Admin cutover stages accept only the next approved stage with required evidence',()=>{
  for(const stage of ['SHADOW_READ','PARITY_VERIFIED','FREEPASS_DATA_READ'] as const){
    assert.equal(parseAdminCutoverApproval(approval(stage),env,stage,NOW).ok,true,stage);
  }
  const skipped=JSON.parse(approval('FREEPASS_DATA_READ')) as Record<string,unknown>;
  skipped.fromStage='OBSERVE';
  assert.equal(parseAdminCutoverApproval(JSON.stringify(skipped),env,'FREEPASS_DATA_READ',NOW).ok,false);
});
...
test('PARITY and final read require approved release evidence and no HOLD reasons',()=>{
  ...
  assert.equal(parseAdminCutoverApproval(JSON.stringify(missing),env,'PARITY_VERIFIED',NOW).ok,false);

  const held=JSON.parse(approval('FREEPASS_DATA_READ')) as any;
  held.holdReasons=['presentation parity incomplete'];
  assert.equal(parseAdminCutoverApproval(JSON.stringify(held),env,'FREEPASS_DATA_READ',NOW).ok,false);
});
...
test('final read requires fallback and production readback evidence',()=>{
  ...
  assert.equal(parseAdminCutoverApproval(JSON.stringify(noFallback),env,'FREEPASS_DATA_READ',NOW).ok,false);
  const noReadback=JSON.parse(approval('FREEPASS_DATA_READ')) as any;
  noReadback.evidence.productionReadbackVerified=false;
  assert.equal(parseAdminCutoverApproval(JSON.stringify(noReadback),env,'FREEPASS_DATA_READ',NOW).ok,false);
});
...
test('cutover approval expires and cannot target a different mode',()=>{
  ...
  assert.equal(parseAdminCutoverApproval(JSON.stringify(expired),env,'PARITY_VERIFIED',NOW).ok,false);
  assert.equal(parseAdminCutoverApproval(approval('PARITY_VERIFIED'),env,'FREEPASS_DATA_READ',NOW).ok,false);
});

test('approved release must still match the live ACTIVE release exactly',()=>{
  const parsed=parseAdminCutoverApproval(approval('PARITY_VERIFIED'),env,'PARITY_VERIFIED',NOW);
