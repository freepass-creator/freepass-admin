...
- Desktop과 Mobile 모두 `상품찾기 / 계약접수 / 전자계약 / 실적관리 / 정산관리` 5축을 같은 순서로 쓴다. 모바일은 `상품 / 접수 / 계약 / 실적 / 정산`으로 줄인다. 활성 탭 전체를 진한 면으로 채우지 않는다. 패널별 실행은 기존 ActionBar와 실제 동작을 유지한다.
...
실적은 접수 원장의 분납실적·완납실적을 거른 보기다. 청구·지급은 정산관리 안에서 가른다.
