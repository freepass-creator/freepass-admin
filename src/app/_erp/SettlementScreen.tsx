  ...
  const grid = (
    <section className="erp-card erp-card--fill">
      <form className="erp-filter" data-region="filter" role="search" action={base}>
        <Field label="정산월" req>
          <select className="erp-input" name="month" defaultValue={month}>{months.map((m) => <option key={m} value={m}>{m}</option>)}</select>
        </Field>
        <Field label={`${who} 이름`}><input className="erp-input" name="gq" defaultValue={sp(q.gq)} placeholder={`${who} 이름`} /></Field>
        <input type="hidden" name="tab" value={tab} />
        <div className="erp-filter-actions">
          <Link className="erp-btn erp-btn--ghost" href={hrefWith(base, {}, { tab: tab === 'pay' ? 'pay' : null })}>초기화</Link>
      ...
      <div className="erp-toolbar" data-region="grid-toolbar">
        ...
        {groups.slice(0, 6).map((g) => (
          <Link key={g.party} className="erp-chip" href={hrefWith(base, q, { g: g.party })} aria-current={gSel?.party === g.party ? 'true' : undefined}>
            {g.party} · {won0(g.net)}
          </Link>
        ))}
      </div>
          ...
          <tbody>
            {lines.map(({ row: r, amount, broken, ratio, party }) => (
              <tr key={`${party}-${r.id}`} data-href={`/intake?ic=${encodeURIComponent(r.id)}`}>
                ...
                <td>{party}</td><td>{txt(r.customer)}</td>
