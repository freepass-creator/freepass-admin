/**
 * ★옮겨 적은 것 — 원본 = fp-settlement `lib/domain/facet-standing.ts` (화이트라벨 · 레트로와 «같은 규칙»).
 *   대표 2026-09-18 「세부검색은 이미 화이트라벨이나 레트로 화면에 만들어 놓은 필터」 — 새로 짓지 않고 옮겨 적는다.
 *   ⚠ 여기서 먼저 고치지 않는다. 원본이 바뀌면 원본대로 다시 옮긴다.
 */
/**
