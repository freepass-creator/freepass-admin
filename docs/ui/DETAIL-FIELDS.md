...
### 환수 대상은 «고르는 것» 이다 — 만드는 것이 아니다

> 대표: 「환수는 누르면 **기존 계약 실적 중에서만 차량번호** 누를 수 있게끔 하자」
...
계약접수 ⋯ → 「환수 실적 만들기…」
   ...
   ┌─ 환수 대상 고르기 ───────────────────────────────┐
   │ 🔍 차량번호 · 고객 · 공급사                        │
   │ ★인도가 찍혀 «실적이 선» 건만 나옵니다             │
   ├──────────────────────────────────────────────┤
   │ 차량번호     고객   차량      공급사  채널  금액  실적 │
   │ 인천 41버 7790 김도호 싼타페MX5 한빛  다이렉트 72만 S-018│
   │ 서울 33다 9012 최윤호 싼타페MX5 대영  한결   45만 S-017│
   │ 경기 05러 6621 한지우 싼타페TM  새턴  카링크  19만 [환수됨]│ ← 잠김
   └──────────────────────────────────────────────┘
```

- **차량번호가 첫 칸**이다 — F04 원장의 열쇠가 차량번호라서다
- 목록은 **정상 실적만**. 환수 줄은 애초에 안 나온다
- **이미 환수가 붙은 줄은 잠근다.** 눌러도 안 열린다
- 고르면 원실적 금액 그대로 부호만 뒤집어 한 줄이 서고, 바로 그 환수 상세가 열린다
- 들어가는 문이 둘 — `계약접수 ⋯` 과 `정산관리 실적목록의 + 환수`

★**접수에 매이지 않는다.** 환수를 만들 때 어느 접수를 보고 있었는지는 상관없다.
차량번호로 고르기 때문이다.
