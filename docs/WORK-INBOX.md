...
## 14. ★디자인 확정 — 2026-09-18 · **더 바꾸지 않는다**

대표(2026-09-18, `localhost:3000` 을 보고): 「나쁘지 않네 디자인… 심플하고」 · 「**그냥 이거로 확정하자**」
「그냥 지친다… 뭐라도 얼른 해야 하는데 **자꾸 바뀌고**」

- **정본 = 지금 `localhost:3000` 에 떠 있는 화면 그대로.** Inter/Pretendard · 흰 작업면 · 떠 있는 판 셋(상품 목록 | 상품 상세 | 접수 목록) · 검은 주 단추.
- ★**새 디자인 방향을 제안하지 않는다.** 레트로·민트·카드 결 바꾸기 등 «옷 갈아입히기»는 끝났다. 다듬기는 이 모습 안에서만.
- 우선은 **기능**이다(§13-5 그대로). 화면을 고치느라 접수·정산 동작을 미루지 않는다.
- ⓘ 정산 워크스테이션(`C:\dev\fp-settlement`, 고전 레트로 결)은 **지금 모습 그대로 둔다** — 흔들지 않는다.
  이 옷으로 옮길지는 따로 정할 일이고, 정하기 전에는 손대지 않는다.
- ⓘ §13-3 「0건 값은 세우지 않는다」는 이 문서를 쓴 Claude 세션(2026-09-16)의 옮겨 적기였고,
  관리자 화면에는 대표 2026-09-10 지시(「옆에다 0이라고 해줘야지」)가 우선한다 — 값은 세우고 숫자만 0.
