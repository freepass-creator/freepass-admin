...
export function OfferPicker({ offers, initial, perks, perksNote }: {
  ...
  return (
    <>
      <h3 className="dz-section-title">계약조건 선택</h3>
      {줄.length === 0 ? <EmptyState>받은 요금이 없습니다 — 원자에 요금이 안 들어온 것입니다.</EmptyState> : (
        ...
        <div className="dz-apply">
          <div className="offer-picker dz-offer-cards" data-ai-feature="data.list-presentation data.variant-selector" data-ai-list-mode="variant-card">
            {줄.map((x) => (
              <button key={x.id} type="button" onClick={() => set고름(x.id)} className={x.id === 고름 ? 'active' : ''}>
                <strong>{x.termMonths}개월 · 월 {원(x.monthlyRent)}</strong>
                <small>{x.deposit === 0 ? '무보증' : `보증금 ${원(x.deposit)}`} · {주행(x.annualMileageKm)}{겹침(x.termMonths) && x.annualMileageKm ? ` · ${x.annualMileageKm / 10000}만km` : ''}</small>
              </button>
            ))}
          </div>
